...
use gpu_dialect::{GpuPod, pod_slice_as_bytes};
...
impl HeadlessDevice {
    ...
    pub fn pool_reserve<T: GpuPod>(
        ...
        let replacement = self.allocate_pool_buffer::<T>("GPU Dialect pool growth", new_capacity);
        let copied_bytes = pool.len as u64 * u64::from(T::LAYOUT.stride);
        let mut encoder = self
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("GPU Dialect pool growth encoder"),
            });
        if copied_bytes > 0 {
